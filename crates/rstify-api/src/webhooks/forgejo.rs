...
pub fn parse_forgejo_event(event_type: &str, body: &[u8]) -> WebhookMessageOutput {
    ...
    let sender = payload.sender.as_ref().and_then(|s| s.login.as_deref()).unwrap_or("unknown");
...
fn parse_pull_request(p: &Payload, repo: &str, sender: &str) -> WebhookMessageOutput {
    ...
    let pr_num = url.as_deref().and_then(|u| u.rsplit('/').next()).unwrap_or("?");
...
fn parse_fork(p: &Payload, repo: &str, sender: &str) -> WebhookMessageOutput {
    let fork_name = p.forkee.as_ref().and_then(|f| f.full_name.as_deref()).unwrap_or("unknown");
...
fn parse_issue_comment(p: &Payload, repo: &str, sender: &str) -> WebhookMessageOutput {
    ...
    let body = format!(
        ...
        sender, action, number, repo, truncate(comment_body, 300)
    );
